...
m = initialization(conf, train=opt.cache)[0]  # 进入初始化，模型加载，图像序列载入

with open('/home/cgn/Desktop/trainacc.csv', 'w', encoding='utf-8', newline='') as f:
    data = ['NM', 'BG', 'CL', 'iter']
    writer = csv.writer(f)
...
for i in range(test_iter):
    ...
    m.load(iter)
    time = datetime.now()
    test = m.transform('train', opt.batch_size)  # 图片跑网络
    ...
    with open('/home/cgn/Desktop/trainacc.csv', 'a', encoding='utf-8') as f:
